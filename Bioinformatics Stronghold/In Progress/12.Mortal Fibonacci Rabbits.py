...
def rabbitFibonacci(n):
    ...
    # 1 breading pairs in first month
    if n==1:
        return 1
    # 1 breeding pairs in second month
    elif n==2:
        return 1
    # breading pairs in subsequent months
    elif lifespan < n:
        (rabbitFibonacci(n-1) + (rabbitFibonacci(n-2)) * kits)
    elif lifespan >= n:
        return ((rabbitFibonacci(n-1)+(rabbitFibonacci(n-2))*kits)-rabbitFibonacci(n-lifespan))


# call rabbit calculating function
